...
ssh $prod_host "pg_dump -h localhost -Fc -f /tmp/db-dump.sql -U postgres with_wagtail_api -x -O"
...
ssh $stage_host "sudo -u postgres pg_restore --clean -h localhost -d with_wagtail_api -U postgres '/tmp/db-dump.sql'"
...
aws --profile with_wagtail_api_devops s3 sync s3://s3.stage.example.com s3://s3.example.com --acl public-read
