...
readonly SSH_HOST=devops@example.com
readonly REMOTE_MEDIA_PATH=/mnt/persist/www/with_wagtail_api/shared/media

scripts_dir="$(cd "$( dirname "${BASH_SOURCE[0]}" )" >/dev/null 2>&1 && pwd)"
docker_dir=${scripts_dir}/../docker

echo "Creating database dump from prod..."
ssh $SSH_HOST "export PGUSER=postgres && pg_dump with_wagtail_api --no-owner > /tmp/db-dump.sql"
