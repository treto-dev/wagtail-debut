...
ssh $prod_host "pg_dump -h localhost -Fc -f /tmp/db-dump.sql -U postgres with_grapple -x -O"
...
ssh $stage_host "sudo -u postgres pg_restore --clean -h localhost -d with_grapple -U postgres '/tmp/db-dump.sql'"
...
aws --profile with_grapple_devops s3 sync s3://s3.stage.example.com s3://s3.example.com --acl public-read
